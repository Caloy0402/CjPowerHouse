    ...
    <div class="app">
        ...
        <form id="deliveryForm" class="delivery-form" action="place_order.php" method="POST">
            ...
            <div class="total-price-section">
                ...
                <!-- Free Shipping Banner -->
                <?php if ($total_amount >= 1500): ?>
                <div class="free-shipping-banner">
                    <span class="banner-icon">🎉</span>
                    FREE SHIPPING APPLIED!
                    <div class="banner-subtitle">
                        Order value: ₱<?php echo number_format($total_amount, 2); ?> (≥ ₱1500)
                    </div>
                </div>
                <?php endif; ?>
                ...
                <div class="price-breakdown">
                    ...
                    <div>
                        ...
                        <span>₱<span id="totalAmount"><?php echo ($total_amount >= 1500) ? number_format($total_amount, 2) : number_format($total_amount + $initial_delivery_fee, 2); ?></span></span>
                    </div>
                </div>
                ...
                <div class="free-shipping-note">
                    ...
                    Add ₱<?php echo number_format(1500 - $total_amount, 2); ?> more to your cart to qualify for <strong>FREE SHIPPING!</strong>
                </div>
                <?php endif; ?>
            </div>
            ...
            <input type="hidden" name="delivery_fee" id="deliveryFeeInput" value="<?php echo ($total_amount >= 1500) ? '0' : htmlspecialchars($initial_delivery_fee); ?>">
            <input type="hidden" name="total_amount_with_delivery" id="totalAmountInput" value="<?php echo ($total_amount >= 1500) ? htmlspecialchars($total_amount) : htmlspecialchars($total_amount + $initial_delivery_fee); ?>">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  form.addEventListener('submit', function(e) {
    ...
    // Validate home description (must not be empty)
    if (!homeDescInput || homeDescInput.value.trim() === '') {
      e.preventDefault();
...
function selectOption(option) {
  ...
  if (option === 'pickup') {
    ...
    // For delivery options, use the barangay fare
    updateDeliveryFee();
  }
...
function updateDeliveryFee() {
  ...
  if (selectedOption && selectedOption.value) {
    ...
    // Calculate and update total amount with free shipping logic
    const subtotal = parseFloat((document.getElementById('subtotal')?.textContent || '0').replace(/,/g, '')) || 0;
    const finalDeliveryFee = subtotal >= 1500 ? 0 : fare;
...
function updateDeliveryFeeDisplay(deliveryFee, subtotal) {
  ...
  const finalDeliveryFee = subtotalAmount >= 1500 ? 0 : deliveryFee;
  const totalAmount = subtotalAmount + finalDeliveryFee;

  // Update delivery fee display
  const deliveryFeeDisplay = document.getElementById('deliveryFeeDisplay');
  if (subtotalAmount >= 1500) {
    deliveryFeeDisplay.innerHTML = '<span style="color: #28a745; font-weight: bold;">FREE</span>';
  } else {
