...
if ($order_id > 0) {
    $sql_order_info = "SELECT total_price, delivery_fee, total_amount_with_delivery, user_id FROM orders WHERE id = ?";
    $stmt_order_info = $conn->prepare($sql_order_info);
    
    if ($stmt_order_info) {
        $stmt_order_info->bind_param("i", $order_id);
        $stmt_order_info->execute();
        $stmt_order_info->bind_result($fetched_total_price, $fetched_delivery_fee, $fetched_total_with_delivery, $associated_user_id);
        $stmt_order_info->fetch();
        $stmt_order_info->close();

        if ($fetched_total_price !== null) { $total_price = (float)$fetched_total_price; }
        if ($fetched_delivery_fee !== null) { $delivery_fee = (float)$fetched_delivery_fee; }
        if ($fetched_total_with_delivery !== null && (float)$fetched_total_with_delivery > 0) {
            $total_with_delivery = (float)$fetched_total_with_delivery;
        } else {
        ...
        if ($associated_user_id) {
            ...
            if ($stmt_user_phone) {
                $stmt_user_phone->bind_param("i", $associated_user_id);
                ...
                if ($fetched_phone_number !== null) {
                 ...
                 error_log("Payment success page: Failed to prepare user phone fetch statement: " . $conn->error);
            }
        }
    } else {
        error_log("Payment success page: Failed to prepare order fetch statement: " . $conn->error);
    }
}

if ($order_id === 0 || $total_with_delivery === 0.00) {
    $order_id = 999; 
    $total_price = 100.00;
    $delivery_fee = 0.00;
