...
// Check if a specific barangay is selected from the GET parameters
...
// Construct the base SQL query to fetch PENDING/PAYMENT-PROCESSING GCASH orders
$sql = "SELECT o.id, t.transaction_number, o.order_date,
               ...
               COALESCE(bf.fare_amount, 0) AS barangay_fare, COALESCE(bf.staff_fare_amount, 0) AS barangay_staff_fare
        FROM orders o
        ...
        WHERE o.payment_method = 'GCASH' AND (o.order_status = 'Pending Payment' OR o.order_status = 'Processing')";

// If a specific barangay is selected, add it to the WHERE clause
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $output .= '<tr>
            ...
            <td>' . htmlspecialchars($row['order_status'] === 'Pending Payment' ? 'Processing' : $row['order_status']) . '</td>
            <td>
    ...
    // Corrected the message to reflect GCASH orders
    $output = '<tr><td colspan="8">No pending GCASH orders found.</td></tr>';
}
